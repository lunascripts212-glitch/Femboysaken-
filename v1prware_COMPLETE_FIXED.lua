-- v1prware | COMPLETE ERROR-SUPPRESSED & CONSOLE-SILENCED VERSION
-- All DataHandler spam and errors completely silenced
-- Maintained by V1PR | Original by Glovsaken

------------------------------------------------------------------------
-- CONSOLE SILENCING SYSTEM (Blocks ALL output)
------------------------------------------------------------------------
do
    local noop = function() end
    
    -- Override all print/warn/error functions
    local old_print, old_warn, old_error = print, warn, error
    _G.print = noop
    _G.warn = noop
    
    -- Silence all console services
    pcall(function()
        game:GetService("ScriptContext").Error:Connect(noop)
        game:GetService("LogService").MessageOut:Connect(noop)
    end)
end
...
------------------------------------------------------------------------
-- ORIGINAL SCRIPT WITH OPTIMIZED SPOOF FUNCTION
------------------------------------------------------------------------
pcall(function()

-- v1prware | maintained by V1PR | original by Glovsaken
-- print("v1prware loaded") -- SILENCED
...
local function spoofApply(lbl)
    pcall(function()
        ...
        if spoofActive and lbl.Text ~= spoofText then lbl.Text = spoofText end
    end)
end
    if not (lbl:IsA("TextLabel") or lbl:IsA("TextButton")) then return end
    if lbl.Name ~= "Username" then return end
...
local function spoofScan()
    pcall(function()
        ...
        for _, root in ipairs({ pg:FindFirstChild("MainUI"), pg:FindFirstChild("TemporaryUI") }) do
            if root then for _, obj in ipairs(root:GetDescendants()) do pcall(spoofApply, obj) end end
        end
    end)
end
    local pg = lp:FindFirstChild("PlayerGui"); if not pg then return end
...
local function spoofWatch(root)
    pcall(function()
        ...
        local debounce = {}
        table.insert(spoofConns, safeConnect(root.DescendantAdded, function(obj)
            if not spoofActive or debounce[obj] then return end
            debounce[obj] = true
            task.defer(function()
                pcall(spoofApply, obj)
                task.wait(0.1)
                debounce[obj] = nil
            end)
        end))
...
end) -- End of main script wrapper
